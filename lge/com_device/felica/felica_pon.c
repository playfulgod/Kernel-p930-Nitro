...
static int felica_pon_open (struct inode *inode, struct file *fp)
{
  ...
  FELICA_DEBUG_MSG("[FELICA] felica_pon_open - result_open(%d) \n",result_open_pon);
...
#ifdef FELICA_FN_DEVICE_TEST
    FELICA_DEBUG_MSG("[FELICA] felica_pon_write - result_write_pon(%d) \n",result_write_pon);
...
#ifdef FUNCTION_DRIVER_SECUIRY
  if(0 == felica_check_uid_mfc())return 0;
#endif
	
  FELICA_DEBUG_MSG("[FELICA_PON] felica_pon_release current_uid : %d \n",current_uid());
...
#ifdef FELICA_FN_DEVICE_TEST
  FELICA_DEBUG_MSG("[FELICA] felica_pon_close - result_close_pon(%d) \n",result_close_pon);
